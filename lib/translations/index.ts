// 完全な多言語翻訳定義
export const translations = {
  ja: {
    // ナビゲーション
    nav: {
      home: 'ホーム',
      propertyInfo: '物件情報',
      rent: '賃貸',
      sale: '売買',
      invest: '投資物件',
      properties: '投資物件',
      management: '管理',
      minpaku: '民泊',
      market: 'マーケット',
      about: '会社概要',
      philosophy: '企業理念',
      contact: 'お問い合わせ',
    },
    
    // ヒーローセクション
    hero: {
      companyType: '株式会社',
      title: '物心両面の幸福と利他の心で、',
      subtitle: '世界に通じる価値を創造する',
      cta: '物件を探す',
      learnMore: '詳しく見る',
      searchRent: '賃貸物件を探す',
      searchSale: '売買物件を探す',
      searchInvest: '投資物件を探す',
      searchMinpaku: '民泊運営相談',
      searchPlaceholder: 'エリア、沿線、駅名で検索',
      searchButton: '検索',
    },
    
    // 住まいの物語（建築と自然）
    story: {
      label: 'Our Perspective',
      title: '建築と自然が、静かに溶け合う場所へ',
      lead: '住まいとは、壁と屋根だけではありません。朝の光、風の通り道、窓の外の緑、素材に刻まれる歳月——そのすべてが暮らしをかたちづくります。',
      body: '私たちは物件を「箱」としてではなく、時間とともに育つ「場」として捉えます。光と影の移ろい、水や植物の気配、手に触れる素材の肌理。そうした細部に目を向けながら、お客様一人ひとりの暮らしに寄り添う住まいをご提案します。',
      item1Title: '光と影',
      item1Text: '一日の光の移ろいが、部屋の表情を静かに変えていきます。',
      item2Title: '素材と歳月',
      item2Text: '木や石は、年月を経るほどに深い味わいをまといます。',
      item3Title: '緑と水',
      item3Text: '窓辺の緑、水の気配。自然は暮らしに呼吸をもたらします。',
      caption1: '木漏れ日 — 時間の痕跡',
      caption2: '素材の肌理 — 歳月が刻む表情',
      caption3: '苔むす石 — 静かに積み重なる時間',
    },

    // 建築と人の対話 — 空間の哲学
    dialogue: {
      label: 'Architecture & People',
      title: '建築と人は、\n静かに語り合う',
      lead: '空間は、そこに生きる人の気配を映します。誰かが置いた一脚の椅子、ふと生けられた一枝の緑。建築と人と自然のあいだに生まれる、声なき対話に耳を澄ませる。',
      items: [
        {
          eyebrow: '人の痕跡 — 余白',
          title: '誰かが、ここにいた',
          text: '一脚の椅子、掛けられた一枚の布。人が去ったあとも、空間には温もりと気配が残ります。余白とは、その記憶が呼吸する場所です。',
        },
        {
          eyebrow: '心と空間の対話',
          title: '小さな庭に、大きな自然を見る',
          text: '掃き清められた砂の波紋、静かに佇む石。手を加えることで、かえって自然の広がりが立ち上がる。内省へと導く、秩序のなかの静けさ。',
        },
        {
          eyebrow: '素材の誠実 — 歳月',
          title: '石は語らず、時を刻む',
          text: '磨かれない石の肌理、光が描く陰影。素材はありのままの姿で、地質の時間と歳月の重みを静かに物語ります。',
        },
        {
          eyebrow: '設計と自然の共生',
          title: '削るほど、余白が生きる',
          text: '整えられた器と、割れた石、芽吹く一枝。人の手による意匠と、自然のほころびが響き合う。最小限の介入が、かたちを引き立てます。',
        },
      ],
    },

    // サービス
    services: {
      title: '事業内容',
      subtitle: '4つの事業で、お客様の幸福を実現します',
      learnMore: '詳しく見る',
      dialogueLead: '事業のかたちは違っても、私たちが見つめるのはいつも同じ——建築と人が交わす、静かな対話です。',
      rent: {
        title: '賃貸事業',
        philosophy: '人と、はじまりの場所をつなぐ',
        description: 'エリア・沿線・賃料・間取りなど、お客様のニーズに合わせた物件検索サービス',
      },
      sale: {
        title: '売買事業',
        philosophy: '暮らしの器を、次の人生へ受け渡す',
        description: 'マンション・戸建・土地・投資用物件まで、幅広い不動産売買のサポート',
      },
      management: {
        title: '管理事業',
        philosophy: '歳月とともに、住まいの価値を守り育てる',
        description: '賃貸物件の総合管理サービスで、オーナー様の資産価値を守ります',
      },
      minpaku: {
        title: '民泊事業',
        philosophy: '訪れる人と、この土地の時間を結ぶ',
        description: 'OneStep PMS連動で、民泊運営を完全サポート。収益最大化を実現',
      },
      stats: {
        minpakuProperties: '民泊運営物件',
        managedProperties: '管理物件',
        customerSmiles: 'お客様の笑顔',
      },
    },
    
    // 企業理念
    philosophy: {
      title: '企業理念',
      subtitle: '私たちの行動指針と目指すべき姿',
      heroTitle: '企業理念',
      heroSubtitle: '物心両面の幸福と利他の心で、世界に通じる価値を創造する',
      missionCardTitle: '物心両面の幸福を追求',
      missionCardDesc: 'お客様と従業員、そして社会全体の物質的・精神的な豊かさを実現し、すべてのステークホルダーの幸福に貢献します。',
      missionCheck1: 'お客様の満足と幸福を最優先',
      missionCheck2: '従業員の成長と働きがいの実現',
      missionCheck3: '社会への貢献と価値提供',
      visionCardTitle: '利他の心で価値創造',
      visionCardDesc: '利他の心を持って、世界に通じる価値を創造し、不動産業界のリーディングカンパニーを目指します。',
      visionCheck1: '他者を思いやる心を大切に',
      visionCheck2: '革新的なサービスの提供',
      visionCheck3: '業界のスタンダードを創造',
      guidelinesTitle: '行動指針',
      guideline1Title: '誠実さ',
      guideline1Desc: 'すべての行動において、誠実であることを第一とします。お客様、取引先、社会に対して正直で透明性のある対応を心がけます。',
      guideline2Title: '挑戦',
      guideline2Desc: '現状に満足せず、常に新しい価値の創造に挑戦します。失敗を恐れず、学び続ける姿勢を大切にします。',
      guideline3Title: '感謝',
      guideline3Desc: 'すべての出会いとご縁に感謝の気持ちを持ちます。お客様、仲間、社会への感謝を忘れません。',
      promisesTitle: '私たちの約束',
      promiseCustomerTitle: 'お客様への約束',
      promiseCustomerItems: [
        '最高のサービスと満足を提供します',
        'お客様の立場に立った提案を行います',
        '迅速かつ丁寧な対応を心がけます',
        '長期的な関係構築を大切にします',
      ],
      promiseSocietyTitle: '社会への約束',
      promiseSocietyItems: [
        '法令遵守と倫理的な経営を実践します',
        '環境に配慮した事業活動を推進します',
        '地域社会の発展に貢献します',
        '持続可能な社会の実現に取り組みます',
      ],
      promiseEmployeeTitle: '従業員への約束',
      promiseEmployeeItems: [
        '働きがいのある職場環境を提供します',
        '公正な評価と成長機会を用意します',
        'ワークライフバランスを尊重します',
        '多様性を認め、個性を活かします',
      ],
      ourPhilosophyTitle: '私たちの理念',
      motto: '誠意正心 知行合一',
      mottoDescription: '誠実な心を正しく保ち、学んだことを必ず実践する',
      vision: 'ビジョン',
      visionText: '世界で戦えるブランドへの飛躍、永続的発展の実現',
      mission: 'ミッション',
      missionText: '他社を凌駕する速さ・安さ・便利さでお客様の笑顔を創造する',
      values: '価値観',
      principlesTitle: '経営理念 7つの原則',
      principles: [
        '全従業員の幸福追求',
        '人として正しく',
        'Win-Winの関係',
        '売上最大・経費最小',
        '全員参加経営',
        '誰にも負けない努力',
        '明るく前向きに',
      ],
      missionPoints: [
        '深い洞察でニーズを理解',
        '速く・安く・便利に提供',
        '感動体験とファンの創造',
      ],
      visionPoints: [
        'グローバルブランドへ',
        '100年続く企業基盤',
        '常に進化する組織',
      ],
      valuesPoints: [
        '物心両面の幸福追求',
        '利他の心で社会貢献',
        '誰にも負けない努力',
      ],
      practiceTitle: '経営理念の実践',
      practiceDescription: '私たちは、この理念を日々の業務の中で実践し、お客様、社会、従業員すべての幸福を追求します。',
      practiceStats: {
        satisfaction: '顧客満足度目標',
        support: 'サポート体制',
        languages: '対応可能',
        growth: '成長の可能性',
      },
      ctaTitle: '私たちと一緒に働きませんか',
      ctaDescription: 'KANAEの理念に共感し、一緒に価値を創造していただける方を募集しています。',
      ctaContact: 'お問い合わせ',
      ctaAbout: '会社概要を見る',
    },
    
    // 会社概要
    about: {
      title: '会社概要',
      subtitle: '物心両面の幸福と利他の心で、世界に通じる価値を創造する',
      heroTitle: '会社概要',
      heroSubtitle: '物心両面の幸福と利他の心で、世界に通じる価値を創造する',
      companyInfoTitle: '会社情報',
      strengthsTitle: '当社の強み',
      businessContentTitle: '事業内容',
      service1Title: '賃貸事業',
      service1Desc: 'お客様のライフスタイルに合わせた最適な賃貸物件をご提案します。',
      service2Title: '売買事業',
      service2Desc: '不動産の売買仲介から投資用物件のご紹介まで幅広く対応します。',
      service3Title: '管理事業',
      service3Desc: 'オーナー様の大切な資産を適切に管理し、安定した収益をサポートします。',
      service4Title: '民泊事業',
      service4Desc: '民泊運営のノウハウを活かし、収益最大化をサポートします。',
      ourPhilosophyTitle: '私たちの理念',
      ourPhilosophySubtitle: '物心両面の幸福と利他の心で、世界に通じる価値を創造する',
      ourPhilosophyButton: '企業理念を見る',
      accessTitle: 'アクセス',
      locationTitle: '所在地',
      accessDetailTitle: 'アクセス',
      contactSectionTitle: 'お問い合わせ',
      contactSectionDesc: '物件に関するご質問や、サービスについてのお問い合わせは、お気軽にご連絡ください。',
      contactButton: 'お問い合わせはこちら',
      companyInfo: '会社情報',
      companyName: '会社名',
      representative: '代表者',
      established: '設立',
      corporateNumber: '会社法人等番号',
      address: '本社所在地',
      phone: '電話番号',
      email: 'メールアドレス',
      website: '公式サイト',
      businessHours: '営業時間',
      businessHoursValue: '平日 9:00〜18:00 / 土曜 10:00〜17:00（日祝休業）',
      businessContent: '事業内容',
      businessContentValue: '不動産賃貸仲介・売買仲介・賃貸管理・民泊運営・不動産コンサルティング',
      licenseNumber: '許認可番号',
      licenseNumberValue: '宅地建物取引業 東京都知事(1)第107157号',
      associations: '加盟団体',
      associationsValue: '公益社団法人 全国宅地建物取引業協会連合会、公益社団法人 東京都宅地建物取引業協会',
      strengths: '当社の強み',
      strength1Title: '地域密着型の丁寧なサービス',
      strength1Description: '東京23区を中心に、地域の特性を熟知したスタッフが、お客様一人ひとりのニーズに合わせた最適な物件をご提案いたします。',
      strength2Title: '多言語対応（日本語・中国語・英語）',
      strength2Description: '外国籍のお客様も安心してご利用いただけるよう、中国語・英語に対応したスタッフが在籍しております。',
      strength3Title: '最新テクノロジーの活用',
      strength3Description: 'AIを活用した物件マッチングシステムや、オンライン内見など、最新技術を導入し、効率的な物件探しをサポートします。',
      strength4Title: 'ワンストップサービス',
      strength4Description: '賃貸・売買・管理・民泊まで、不動産に関するあらゆるニーズに、一社で対応できる総合力が当社の強みです。',
      access: 'アクセス',
      location: '所在地',
      accessInfo: 'アクセス情報',
      nearStation1: 'JR・東京メトロ 高田馬場駅',
      nearStation1Time: '早稲田口より徒歩約10分',
      nearStation2: '東京メトロ副都心線 西早稲田駅',
      nearStation2Time: '3番出口より徒歩約8分',
    },
    
    // 売買検索
    sale: {
      title: '売買物件検索',
      subtitle: 'マンション・一戸建て・土地など、資産価値の高い物件をご提案します',
      searchTitle: '物件を探す',
      propertyType: '物件種別',
      apartment: 'マンション',
      house: '一戸建て',
      land: '土地',
      location: 'エリア',
      locationPlaceholder: 'エリアを選択してください',
      minPrice: '価格（下限）',
      maxPrice: '価格（上限）',
      pricePlaceholder: '例: 30000000',
      searchButton: 'この条件で検索する',
    },
    
    // 民泊
    minpaku: {
      title: '民泊運営代行サービス',
      subtitle: 'お持ちの物件を民泊として運営し、安定した収益を実現します',
      calculator: '収益シミュレーター',
      calculatorDesc: 'お持ちの物件で民泊を運営した場合の収益を簡単にシミュレーションできます',
      propertyType: '物件種別',
      area: '面積（㎡）',
      areaPlaceholder: '50',
      rooms: '間取り',
      nightlyRate: '1泊あたりの料金',
      nightlyRatePlaceholder: '10000',
      occupancyRate: '稼働率（%）',
      occupancyRatePlaceholder: '70',
      managementFee: '管理手数料率（%）',
      managementFeePlaceholder: '25',
      calculate: '収益を計算する',
      results: '収益シミュレーション結果',
      income: '収入',
      grossRevenue: '総売上',
      bookedNights: '予約泊数',
      nights: '泊',
      expenses: '支出',
      managementFeeLabel: '管理手数料',
      cleaningFee: '清掃費',
      utilities: '光熱費',
      platformFee: 'プラットフォーム手数料',
      totalExpenses: '総支出',
      netRevenue: '純利益',
      perMonth: '/ 月',
      servicesTitle: '民泊運営代行サービス内容',
      service1: 'OneStep PMS連動',
      service1Desc: 'Airbnb・Booking.com等と自動連携',
      service2: '24時間ゲスト対応',
      service2Desc: '多言語対応のゲストサポート',
      service3: '清掃・メンテナンス',
      service3Desc: 'プロの清掃チームが対応',
      service4: '収益最適化',
      service4Desc: 'AI価格設定で収益最大化',
    },
    
    // 賃貸検索
    rent: {
      title: '賃貸物件検索',
      subtitle: 'お客様のライフスタイルに合った理想の賃貸物件をお探しします',
      searchTitle: '物件を探す',
      area: 'エリア',
      areaPlaceholder: 'エリアを選択してください',
      minRent: '賃料（下限）',
      maxRent: '賃料（上限）',
      layout: '間取り',
      features: 'こだわり条件',
      searchButton: 'この条件で検索する',
      recommended: 'おすすめ物件',
      perMonth: '/ 月',
      viewDetails: '詳細を見る',
      favorite: 'お気に入り',
      loadMore: 'もっと見る',
      rentalProcess: '賃貸物件ご契約の流れ',
      step1: '物件検索',
      step1Desc: 'ご希望の条件で物件を検索',
      step2: '内見予約',
      step2Desc: '気になる物件の内見を予約',
      step3: '入居申込',
      step3Desc: '審査のため申込書を提出',
      step4: '契約手続き',
      step4Desc: '重要事項説明と契約締結',
      step5: '入居開始',
      step5Desc: '鍵の受け渡し・入居',
    },
    
    // 賃貸管理
    management: {
      title: '賃貸管理サービス',
      subtitle: 'オーナー様と入居者様、双方にとって最適な賃貸管理をご提供します',
      forOwners: 'オーナー様向け',
      forTenants: '入居者様向け',
      ownerServices: 'オーナー様向け管理サービス',
      feature1: '安定した収益管理',
      feature1Desc: '家賃の集金代行から滞納対応まで、収益を確実に確保します。',
      feature2: '建物・設備管理',
      feature2Desc: '定期点検からトラブル対応まで、物件の価値を維持します。',
      feature3: '入居者管理',
      feature3Desc: '入居者募集から契約更新まで、総合的にサポートします。',
      rentManagement: '賃料管理サービス',
      buildingManagement: '建物管理サービス',
      tenantManagement: '入居者管理サービス',
      otherServices: 'その他サービス',
      managementFees: '管理手数料',
      basicPlan: '基本管理プラン',
      fullPlan: 'フル管理プラン',
      premiumPlan: 'プレミアムプラン',
      recommended: 'おすすめ',
      managementStats: '管理実績',
      properties: '管理物件数',
      units: '管理戸数',
      occupancyRate: '入居率',
      avgYears: '平均管理年数',
      tenantServices: '入居者様向けサービス',
      comfortableSupport: '快適な住環境のサポート',
      supportDesc: '入居中のお困りごとから退去時の手続きまで、安心してお住まいいただけるよう全力でサポートいたします。',
      contactWindow: 'お問い合わせ窓口',
      businessHours: '営業時間',
      emergencyContact: '緊急連絡先（24時間）',
      faq: 'よくあるご質問',
      ctaTitle: '管理のご相談はお気軽に',
      ctaDescription: '物件管理に関するご相談は無料です。お気軽にお問い合わせください。',
      ctaButton: 'お問い合わせはこちら',
    },
    
    // CTA（行動呼びかけ）
    cta: {
      title: 'あなたの夢を叶える、',
      subtitle: '最高のパートナーがここに',
      description: '賃貸・売買・管理・民泊まで、全ての不動産ニーズにワンストップで対応。お客様の幸福を第一に、誠実なサービスを提供します。',
      searchProperties: '物件を探す',
    },
    
    // 特徴
    features: {
      title: 'KANAEが選ばれる理由',
      subtitle: 'お客様の幸福を第一に考え、質の高いサービスを提供します',
      items: [
        {
          title: '多言語対応',
          description: '日本語・中国語・英語に対応し、外国人のお客様も安心してご利用いただけます',
          iconPath: 'M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9',
        },
        {
          title: '24時間対応',
          description: 'オンライン問い合わせは24時間受付。緊急時のサポート体制も万全です',
          iconPath: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z',
        },
        {
          title: '安心のサポート',
          description: '宅地建物取引士・一級建築士が在籍。専門知識でお客様をサポートします',
          iconPath: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z',
        },
        {
          title: 'IT化推進',
          description: 'いえらぶ・ITANDI・OneStep PMSとの連携で、スムーズな契約と管理を実現',
          iconPath: 'M13 10V3L4 14h7v7l9-11h-7z',
        },
        {
          title: '透明な料金体系',
          description: '初期費用・税金計算をその場で確認。隠れた費用は一切ありません',
          iconPath: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
        },
        {
          title: 'ワンストップ対応',
          description: '賃貸・売買・管理・民泊まで、全ての不動産ニーズに一社で対応します',
          iconPath: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z',
        },
      ],
    },
    
    // 統計情報
    stats: {
      title: '数字で見るKANAE',
      subtitle: '着実な成長と、確かな実績',
      items: [
        {
          number: '30+',
          label: '民泊運営物件',
          description: 'OneStep PMS連動で高収益を実現',
        },
        {
          number: '500+',
          label: '賃貸・売買管理物件',
          description: '豊富な実績で安心のサポート',
        },
        {
          number: '1000+',
          label: 'お客様の笑顔',
          description: '満足度95%以上の評価',
        },
        {
          number: '3年',
          label: '創業からの実績',
          description: '2021年設立、着実な成長',
        },
      ],
      banksTitle: '主要取引銀行',
      banksSubtitle: '確かな信頼関係で、安心の取引をサポート',
      banks: [
        'SBJ銀行',
        'スター銀行',
        'ソニー銀行',
        'あすか信用組合',
        'SMBC信託銀行',
        '三井住友銀行',
      ],
    },
    
    // お問い合わせ
    contact: {
      title: 'お問い合わせ',
      subtitle: 'お気軽にご相談ください',
      phone: '電話でのお問い合わせ',
      email: 'メールでのお問い合わせ',
      businessHours: '営業時間',
      businessHoursValue: '平日 9:00-18:00',
      businessHoursNote: '(土日祝は予約制)',
      address: '所在地',
      inquiryButton: 'お問い合わせはこちら',
      onlineNote: '※ オンライン相談・内見予約は24時間受付中',
      formTitle: 'お問い合わせフォーム',
      formDescription: '以下のフォームに必要事項をご入力の上、送信してください。担当者より3営業日以内にご連絡いたします。',
      name: 'お名前',
      namePlaceholder: '山田 太郎',
      nameRequired: 'お名前を入力してください',
      emailLabel: 'メールアドレス',
      emailPlaceholder: 'example@email.com',
      emailRequired: 'メールアドレスを入力してください',
      phoneLabel: '電話番号',
      phonePlaceholder: '03-1234-5678',
      phoneOptional: '任意',
      inquiryType: 'お問い合わせ種別',
      inquiryTypeViewing: '内見予約',
      inquiryTypeInquiry: '一般お問い合わせ',
      inquiryTypeApplication: '入居申込',
      inquiryTypeOther: 'その他',
      message: 'お問い合わせ内容',
      messagePlaceholder: 'お問い合わせ内容をご記入ください',
      messageRequired: 'お問い合わせ内容を入力してください',
      submit: '送信する',
      submitting: '送信中...',
      successTitle: '送信完了',
      successMessage: 'お問い合わせありがとうございます。担当者より3営業日以内にご連絡いたします。',
      errorTitle: '送信エラー',
      errorMessage: '送信に失敗しました。お手数ですが、お電話（03-6914-3633）またはメール（info@kanae-tokyo.com）にてご連絡ください。',
      backToHome: 'ホームに戻る',
    },
    
    // フッター
    footer: {
      companyName: '株式会社KANAE',
      slogan: '物心両面の幸福と利他の心で、世界に通じる価値を創造する',
      license: '東京都知事(1)第107157号',
      businessContent: '事業内容',
      rentalBusiness: '賃貸事業',
      salesBusiness: '売買事業',
      managementBusiness: '管理事業',
      minpakuBusiness: '民泊事業',
      companyInfo: '会社情報',
      aboutUs: '会社概要',
      philosophy: '企業理念',
      contact: 'お問い合わせ',
      privacyPolicy: 'プライバシーポリシー',
      contactInfo: 'お問い合わせ',
      terms: '利用規約',
      sitemap: 'サイトマップ',
      allRightsReserved: 'All rights reserved.',
    },
    
    // 投資物件検索
    properties: {
      title: '投資物件情報',
      subtitle: '投資物件を中心に、賃貸・売買・投資物件を一つの窓口でご提案します。',
      label: 'Properties',
      all: 'すべて',
      rent: '賃貸',
      sale: '売買',
      invest: '投資物件',
      searchTitle: '物件を探す',
      searchPlaceholder: 'エリア、沿線、駅名で検索',
      searchButton: '検索',
      filterByType: '物件種別で絞り込む',
      quickSearch: 'クイック検索',
      detailedSearch: '詳細検索',
      showAllProperties: '全物件を表示',
      propertyView: '物件一覧',
      wardView: 'エリア別検索',
      totalResults: '条件に合う物件：',
      resultsCount: '件',
      activeFilters: '個の条件',
      clearAll: '全てクリア',
      geography: '地理位置',
      ward: '区',
      selectWard: '全エリア',
      railway: '路線',
      selectRailway: '全路線',
      station: '駅',
      selectStation: '全駅',
      budget: '価格帯',
      minPrice: '下限（万円）',
      maxPrice: '上限（万円）',
      pricePlaceholderMin: '例：3000',
      pricePlaceholderMax: '例：10000',
      propertyType: '物件種別',
      wholeBuilding: '一棟マンション',
      wholeOffice: '一棟ビル',
      wholeApartment: '一棟アパート',
      incomeProperty: '一棟収益',
      area: '面積',
      landArea: '土地面積（㎡）',
      buildingArea: '建物面積（㎡）',
      areaMin: '下限',
      areaMax: '上限',
      advancedFilters: '詳細条件',
      expectedYield: '想定利回り（%）',
      yieldMin: '下限（例：4.0）',
      yieldMax: '上限（例：10.0）',
      buildingAge: '築年数',
      ageNotSpecified: '指定なし',
      ageWithin5: '5年以内',
      ageWithin10: '10年以内',
      ageWithin20: '20年以内',
      ageWithin30: '30年以内',
      walkTime: '徒歩時間',
      walkNotSpecified: '指定なし',
      walk5min: '5分以内',
      walk10min: '10分以内',
      walk15min: '15分以内',
      walk20min: '20分以内',
      reset: 'リセット',
      sortBy: '並び替え：',
      byPropertyType: '物件種別：',
      monthlyRent: '月額賃料',
      salePrice: '販売価格',
      grossYield: '表面利回り',
      recommended: 'おすすめ物件',
      viewDetails: '詳細を見る',
      favorite: 'お気に入り',
      loadMore: 'もっと見る',
      noResults: '該当する物件が見つかりませんでした',
      rentBadge: '賃貸',
      saleBadge: '売買',
      investBadge: '投資',
      areaLabel: '所在地',
      nearestStation: '最寄駅',
      minutesWalk: '徒歩分',
      layout: '間取り',
      built: '築年月',
      units: '総戸数',
      unitsSuffix: '戸',
    },


    // 投資物業搜索
    // 共通
    common: {
      readMore: '詳しく見る',
      backToTop: 'トップへ戻る',
      loading: '読み込み中...',
      error: 'エラーが発生しました',
      success: '成功しました',
      confirm: '確認',
      cancel: 'キャンセル',
      save: '保存',
      delete: '削除',
      edit: '編集',
      search: '検索',
      close: '閉じる',
    },
  },
  
  zh: {
    // 导航
    nav: {
      home: '首頁',
      propertyInfo: '物件資訊',
      rent: '租賃',
      sale: '買賣',
      invest: '投資物業',
      properties: '投資物業',
      management: '管理',
      minpaku: '民宿',
      market: '市場洞察',
      about: '公司簡介',
      philosophy: '企業理念',
      contact: '聯繫我們',
    },
    
    // 主视觉区
    hero: {
      companyType: '株式會社',
      title: '追求物質與精神的雙重幸福，',
      subtitle: '以利他之心創造通往世界的價值',
      cta: '搜索房源',
      learnMore: '瞭解更多',
      searchRent: '搜索租賃房源',
      searchSale: '搜索買賣房源',
      searchInvest: '搜索投資房源',
      searchMinpaku: '民宿運營諮詢',
      searchPlaceholder: '按地區、線路、車站搜索',
      searchButton: '搜索',
    },
    
    // 居住的故事（建筑与自然）
    story: {
      label: 'Our Perspective',
      title: '讓建築與自然，靜靜地交融',
      lead: '住所不只是牆壁與屋頂。清晨的光線、風的通道、窗外的綠意、材質上鐫刻的歲月——這一切共同構成了生活本身。',
      body: '我們不把房產看作單純的“容器”，而是隨時間生長的“場所”。光影的流轉、水與植物的氣息、指尖觸及的材質肌理。我們關注這些細節，為每一位客戶提供貼近生活本質的居住方案。',
      item1Title: '光與影',
      item1Text: '一天中光線的變化，靜靜地改變着房間的表情。',
      item2Title: '材質與歲月',
      item2Text: '木與石，隨着歲月流轉而愈發溫潤深邃。',
      item3Title: '綠意與水',
      item3Text: '窗邊的綠植、水的氣息，讓生活有了呼吸。',
      caption1: '樹影婆娑 — 時間的痕跡',
      caption2: '材質肌理 — 歲月鐫刻的表情',
      caption3: '苔石靜默 — 悄然沉澱的時光',
    },

    // 建築與人的對話 — 空間哲學
    dialogue: {
      label: 'Architecture & People',
      title: '建築與人，\n靜靜地對話',
      lead: '空間，映照着生活其中的人的氣息。有人擺放的一把椅子，隨手插起的一枝綠意。在建築、人與自然之間，聆聽那無聲的對話。',
      items: [
        {
          eyebrow: '人的痕跡 — 留白',
          title: '曾有人，在此停留',
          text: '一把椅子，一方垂落的布。即使人已離去，空間依舊留存着溫度與氣息。留白，正是這份記憶得以呼吸的地方。',
        },
        {
          eyebrow: '心與空間的對話',
          title: '於方寸之庭，見天地之大',
          text: '掃淨的砂石波紋，靜立的一方頑石。正因有了人的用心，自然的遼闊反而升起。這是秩序之中，引人內省的靜謐。',
        },
        {
          eyebrow: '材質的誠實 — 歲月',
          title: '石不言語，卻鐫刻時光',
          text: '未經打磨的石之肌理，光影勾勒的明暗。材質以本來的樣貌，靜靜訴說着地質的時間與歲月的重量。',
        },
        {
          eyebrow: '設計與自然的共生',
          title: '愈是刪減，留白愈顯生機',
          text: '雅緻的器皿、開裂的石塊、初萌的枝芽。人手的匠意，與自然的殘缺彼此呼應。最低限度的介入，最能襯托形之本真。',
        },
      ],
    },
    
    // 服务
    services: {
      title: '業務內容',
      subtitle: '通過四大業務，實現客戶的幸福',
      learnMore: '瞭解更多',
      dialogueLead: '業務的形態各異，我們凝望的卻始終如一——建築與人之間，那場靜謐的對話。',
      rent: {
        title: '租賃業務',
        philosophy: '為人，連結起點之所',
        description: '根據地區、線路、租金、戶型等，為客戶提供符合需求的房源搜索服務',
      },
      sale: {
        title: '買賣業務',
        philosophy: '將生活的容器，交付予下一段人生',
        description: '從公寓、獨棟、土地到投資用物業，提供全方位的房地產買賣支持',
      },
      management: {
        title: '管理業務',
        philosophy: '伴隨歲月，守護並培育住所的價值',
        description: '提供租賃物業的綜合管理服務，守護業主的資產價值',
      },
      minpaku: {
        title: '民宿業務',
        philosophy: '讓來訪之人，與這片土地的時光相繫',
        description: '通過OneStep PMS聯動，全面支持民宿運營，實現收益最大化',
      },
      stats: {
        minpakuProperties: '民宿運營物業',
        managedProperties: '管理物業',
        customerSmiles: '客戶笑容',
      },
    },
    
    // 企业理念
    philosophy: {
      title: '企業理念',
      subtitle: '我們的行動準則與目標',
      heroTitle: '企業理念',
      heroSubtitle: '追求物質與精神的雙重幸福，以利他之心創造通往世界的價值',
      missionCardTitle: '追求物質與精神的雙重幸福',
      missionCardDesc: '實現客戶、員工以及整個社會的物質與精神雙重富足，為所有利益相關者的幸福做出貢獻。',
      missionCheck1: '將客戶的滿意與幸福置於首位',
      missionCheck2: '實現員工的成長與工作價值',
      missionCheck3: '為社會做出貢獻並創造價值',
      visionCardTitle: '以利他之心創造價值',
      visionCardDesc: '以利他之心，創造通往世界的價值，致力於成為房地產行業的領軍企業。',
      visionCheck1: '珍視體貼他人的心',
      visionCheck2: '提供創新服務',
      visionCheck3: '創造行業標準',
      guidelinesTitle: '行動準則',
      guideline1Title: '誠實',
      guideline1Desc: '在所有行動中，以誠實為第一要義。對客戶、合作夥伴和社會保持誠實、透明的態度。',
      guideline2Title: '挑戰',
      guideline2Desc: '不滿足於現狀，不斷挑戰創造新價值。不懼怕失敗，珍視持續學習的姿態。',
      guideline3Title: '感恩',
      guideline3Desc: '對所有相遇和緣分心存感激。永遠不忘對客戶、同事和社會的感謝。',
      promisesTitle: '我們的承諾',
      promiseCustomerTitle: '對客戶的承諾',
      promiseCustomerItems: [
        '提供最優質的服務和滿意度',
        '從客戶角度出發提供建議',
        '迅速而周到地響應需求',
        '重視長期關係的建立',
      ],
      promiseSocietyTitle: '對社會的承諾',
      promiseSocietyItems: [
        '遵守法規，實踐道德經營',
        '推進環保型業務活動',
        '為地區社會的發展做貢獻',
        '致力於實現可持續發展的社會',
      ],
      promiseEmployeeTitle: '對員工的承諾',
      promiseEmployeeItems: [
        '提供充滿工作價值的職場環境',
        '提供公平的評價和成長機會',
        '尊重工作與生活的平衡',
        '認可多樣性，發揮個人特長',
      ],
      ourPhilosophyTitle: '我們的理念',
      motto: '誠意正心 知行合一',
      mottoDescription: '保持誠實正直的心，必定實踐所學',
      vision: '願景',
      visionText: '成為能在世界競爭的品牌，實現永續發展',
      mission: '使命',
      missionText: '以超越其他公司的速度、價格和便利性，創造客戶的笑容',
      values: '價值觀',
      principlesTitle: '經營理念 7大原則',
      principles: [
        '追求全體員工的幸福',
        '作為人正確行事',
        '實現雙贏關係',
        '銷售最大化，成本最小化',
        '全員參與經營',
        '永不服輸的努力',
        '保持積極樂觀',
      ],
      missionPoints: [
        '深入洞察理解需求',
        '提供快速、實惠、便捷的服務',
        '創造感動體驗與忠實客戶',
      ],
      visionPoints: [
        '成為全球品牌',
        '建立百年企業基礎',
        '持續進化的組織',
      ],
      valuesPoints: [
        '追求物質與精神雙重幸福',
        '以利他之心貢獻社會',
        '永不服輸的努力',
      ],
      practiceTitle: '經營理念的實踐',
      practiceDescription: '我們在日常業務中踐行這一理念，追求客戶、社會、員工全體的幸福。',
      practiceStats: {
        satisfaction: '客戶滿意度目標',
        support: '全年無休支持',
        languages: '多語言服務',
        growth: '成長可能性',
      },
      ctaTitle: '與我們一起工作',
      ctaDescription: '我們正在招募認同KANAE理念，願意一起創造價值的夥伴。',
      ctaContact: '聯繫我們',
      ctaAbout: '查看公司簡介',
    },
    
    // 公司简介
    about: {
      title: '公司簡介',
      subtitle: '追求物質與精神的雙重幸福，以利他之心創造通往世界的價值',
      heroTitle: '公司簡介',
      heroSubtitle: '追求物質與精神的雙重幸福，以利他之心創造通往世界的價值',
      companyInfoTitle: '公司信息',
      strengthsTitle: '我們的優勢',
      businessContentTitle: '業務內容',
      service1Title: '租賃業務',
      service1Desc: '為客戶推薦符合生活方式的最優租賃房源。',
      service2Title: '買賣業務',
      service2Desc: '從房產買賣中介到投資物業介紹，全方位對應。',
      service3Title: '管理業務',
      service3Desc: '妥善管理業主的重要資產，支持穩定收益。',
      service4Title: '民宿業務',
      service4Desc: '運用民宿運營專業知識，支持收益最大化。',
      ourPhilosophyTitle: '我們的理念',
      ourPhilosophySubtitle: '追求物質與精神的雙重幸福，以利他之心創造通往世界的價值',
      ourPhilosophyButton: '查看企業理念',
      accessTitle: '交通方式',
      locationTitle: '地址',
      accessDetailTitle: '交通信息',
      contactSectionTitle: '聯繫我們',
      contactSectionDesc: '如有關於房產或服務的問題，歡迎隨時聯繫我們。',
      contactButton: '點擊此處聯繫',
      companyInfo: '公司信息',
      companyName: '公司名稱',
      representative: '代表人',
      established: '成立日期',
      corporateNumber: '公司法人編號',
      address: '總公司地址',
      phone: '電話號碼',
      email: '電子郵箱',
      website: '官方網站',
      businessHours: '營業時間',
      businessHoursValue: '工作日 9:00〜18:00 / 週六 10:00〜17:00（週日及節假日休息）',
      businessContent: '業務內容',
      businessContentValue: '房地產租賃中介・買賣中介・租賃管理・民宿運營・房地產諮詢',
      licenseNumber: '許可證編號',
      licenseNumberValue: '宅地建物取引業 東京都知事(1)第107157號',
      associations: '加盟團體',
      associationsValue: '公益社團法人 全國宅地建物取引業協會聯合會、公益社團法人 東京都宅地建物取引業協會',
      strengths: '我們的優勢',
      strength1Title: '紮根本地的細緻服務',
      strength1Description: '以東京23區為中心，熟知地區特性的員工將為每一位客戶提供最符合需求的房源推薦。',
      strength2Title: '多語言對應（日語・中文・英語）',
      strength2Description: '為了讓外籍客戶也能安心使用，我們配備了中文和英文對應的員工。',
      strength3Title: '運用最新科技',
      strength3Description: '引入AI驅動的房源匹配系統和在線看房等最新技術，為高效找房提供支持。',
      strength4Title: '一站式服務',
      strength4Description: '從租賃、買賣、管理到民宿，能夠一站式滿足所有房地產需求是我們的綜合實力優勢。',
      access: '交通方式',
      location: '地址',
      accessInfo: '交通信息',
      nearStation1: 'JR・東京地鐵 高田馬場站',
      nearStation1Time: '早稻田出口步行約10分鐘',
      nearStation2: '東京地鐵副都心線 西早稻田站',
      nearStation2Time: '3號出口步行約8分鐘',
    },
    
    // 买卖搜索
    sale: {
      title: '買賣房源搜索',
      subtitle: '從公寓、獨棟、土地等，為您推薦具有資產價值的房源',
      searchTitle: '搜索房源',
      propertyType: '房源類型',
      apartment: '公寓',
      house: '獨棟',
      land: '土地',
      location: '地區',
      locationPlaceholder: '請選擇地區',
      minPrice: '價格（下限）',
      maxPrice: '價格（上限）',
      pricePlaceholder: '例: 30000000',
      searchButton: '按此條件搜索',
    },
    
    // 民宿
    minpaku: {
      title: '民宿運營代理服務',
      subtitle: '將您的房產作為民宿運營，實現穩定收益',
      calculator: '收益模擬器',
      calculatorDesc: '可以簡單模擬將您的房產作為民宿運營的收益',
      propertyType: '房源類型',
      area: '面積（㎡）',
      areaPlaceholder: '50',
      rooms: '戶型',
      nightlyRate: '每晚收費',
      nightlyRatePlaceholder: '10000',
      occupancyRate: '入住率（%）',
      occupancyRatePlaceholder: '70',
      managementFee: '管理費率（%）',
      managementFeePlaceholder: '25',
      calculate: '計算收益',
      results: '收益模擬結果',
      income: '收入',
      grossRevenue: '總營收',
      bookedNights: '預訂天數',
      nights: '晚',
      expenses: '支出',
      managementFeeLabel: '管理費',
      cleaningFee: '清掃費',
      utilities: '水電費',
      platformFee: '平台手續費',
      totalExpenses: '總支出',
      netRevenue: '淨利潤',
      perMonth: '/ 月',
      servicesTitle: '民宿運營代理服務內容',
      service1: 'OneStep PMS聯動',
      service1Desc: 'Airbnb・Booking.com等自動聯動',
      service2: '24小時客人服務',
      service2Desc: '多語言客人支持',
      service3: '清掃・維護',
      service3Desc: '專業清掃團隊',
      service4: '收益優化',
      service4Desc: 'AI價格設定實現收益最大化',
    },
    
    // 租赁搜索
    rent: {
      title: '租賃房源搜索',
      subtitle: '為您尋找符合生活方式的理想租賃房源',
      searchTitle: '搜索房源',
      area: '地區',
      areaPlaceholder: '請選擇地區',
      minRent: '租金（下限）',
      maxRent: '租金（上限）',
      layout: '戶型',
      features: '特定條件',
      searchButton: '按此條件搜索',
      recommended: '推薦房源',
      perMonth: '/ 月',
      viewDetails: '查看詳情',
      favorite: '收藏',
      loadMore: '查看更多',
      rentalProcess: '租賃合同流程',
      step1: '房源搜索',
      step1Desc: '按照期望條件搜索房源',
      step2: '預約看房',
      step2Desc: '預約心儀房源的看房',
      step3: '入住申請',
      step3Desc: '提交申請表進行審查',
      step4: '簽約手續',
      step4Desc: '重要事項說明與簽約',
      step5: '開始入住',
      step5Desc: '鑰匙交接・入住',
    },
    
    // 租赁管理
    management: {
      title: '租賃管理服務',
      subtitle: '為業主和租客雙方提供最優質的租賃管理服務',
      forOwners: '面向業主',
      forTenants: '面向租客',
      ownerServices: '業主管理服務',
      feature1: '穩定的收益管理',
      feature1Desc: '從租金代收到欠款催繳，確保收益安全。',
      feature2: '建築・設備管理',
      feature2Desc: '從定期檢查到故障處理，維護物業價值。',
      feature3: '租客管理',
      feature3Desc: '從招租到合同更新，提供全面支持。',
      rentManagement: '租金管理服務',
      buildingManagement: '建築管理服務',
      tenantManagement: '租客管理服務',
      otherServices: '其他服務',
      managementFees: '管理費用',
      basicPlan: '基礎管理方案',
      fullPlan: '全面管理方案',
      premiumPlan: '高級管理方案',
      recommended: '推薦',
      managementStats: '管理實績',
      properties: '管理物業數',
      units: '管理戶數',
      occupancyRate: '入住率',
      avgYears: '平均管理年數',
      tenantServices: '租客服務',
      comfortableSupport: '舒適居住環境支持',
      supportDesc: '從入住期間的困擾到退租手續，全力支持讓您安心居住。',
      contactWindow: '諮詢窗口',
      businessHours: '營業時間',
      emergencyContact: '緊急聯繫方式（24小時）',
      faq: '常見問題',
      ctaTitle: '歡迎諮詢管理服務',
      ctaDescription: '關於物業管理的諮詢免費提供。歡迎隨時聯繫我們。',
      ctaButton: '點擊此處諮詢',
    },
    
    // 行动号召
    cta: {
      title: '實現您的夢想，',
      subtitle: '最佳合作夥伴就在這裡',
      description: '從租賃、買賣、管理到民宿，一站式滿足所有房地產需求。以客戶幸福為第一，提供誠信服務。',
      searchProperties: '搜索房源',
    },
    
    // 特色优势
    features: {
      title: '選擇KANAE的理由',
      subtitle: '以客戶幸福為首要，提供高質量服務',
      items: [
        {
          title: '多語言服務',
          description: '支持日語、中文、英語，外籍客戶也可安心使用',
          iconPath: 'M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9',
        },
        {
          title: '24小時響應',
          description: '在線諮詢24小時受理，緊急時刻支援體系完善',
          iconPath: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z',
        },
        {
          title: '安心支持',
          description: '配備持證房產交易士及一級建築師，以專業知識支持客戶',
          iconPath: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z',
        },
        {
          title: 'IT化推進',
          description: '與ITANDI・OneStep PMS等平台聯動，實現順暢合約與管理',
          iconPath: 'M13 10V3L4 14h7v7l9-11h-7z',
        },
        {
          title: '透明收費體系',
          description: '當場確認初期費用及稅金計算，無任何隱藏費用',
          iconPath: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
        },
        {
          title: '一站式服務',
          description: '從租賃、買賣、管理到民宿，一家公司滿足所有房地產需求',
          iconPath: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z',
        },
      ],
    },
    
    // 统计数据
    stats: {
      title: '用數字看KANAE',
      subtitle: '穩步成長，確實業績',
      items: [
        {
          number: '30+',
          label: '民宿運營物業',
          description: 'OneStep PMS聯動實現高收益',
        },
        {
          number: '500+',
          label: '租賃・買賣管理物業',
          description: '豐富實績，安心支持',
        },
        {
          number: '1000+',
          label: '客戶滿意案例',
          description: '滿意度95%以上',
        },
        {
          number: '3年',
          label: '創業以來實績',
          description: '2021年成立，穩步成長',
        },
      ],
      banksTitle: '主要交易銀行',
      banksSubtitle: '穩固的信賴關係，支持安心交易',
      banks: [
        'SBJ銀行',
        'スター銀行',
        '索尼銀行',
        '飛鳥信用組合',
        'SMBC信託銀行',
        '三井住友銀行',
      ],
    },
    
    // 联系我们
    contact: {
      title: '聯繫我們',
      subtitle: '歡迎隨時諮詢',
      phone: '電話諮詢',
      email: '郵件諮詢',
      businessHours: '營業時間',
      businessHoursValue: '工作日 9:00-18:00',
      businessHoursNote: '（週末及節假日需預約）',
      address: '地址',
      inquiryButton: '點擊諮詢',
      onlineNote: '※ 在線諮詢・看房預約24小時受理',
      formTitle: '諮詢表單',
      formDescription: '請填寫以下表單並提交。我們的工作人員將在3個工作日內與您聯繫。',
      name: '姓名',
      namePlaceholder: '張三',
      nameRequired: '請輸入姓名',
      emailLabel: '郵箱地址',
      emailPlaceholder: 'example@email.com',
      emailRequired: '請輸入郵箱地址',
      phoneLabel: '電話號碼',
      phonePlaceholder: '03-1234-5678',
      phoneOptional: '選填',
      inquiryType: '諮詢類型',
      inquiryTypeViewing: '預約看房',
      inquiryTypeInquiry: '一般諮詢',
      inquiryTypeApplication: '入住申請',
      inquiryTypeOther: '其他',
      message: '諮詢內容',
      messagePlaceholder: '請輸入諮詢內容',
      messageRequired: '請輸入諮詢內容',
      submit: '提交',
      submitting: '提交中...',
      successTitle: '提交成功',
      successMessage: '感謝您的諮詢。我們的工作人員將在3個工作日內與您聯繫。',
      errorTitle: '提交失敗',
      errorMessage: '提交失敗。請通過電話（03-6914-3633）或郵件（info@kanae-tokyo.com）與我們聯繫。',
      backToHome: '返回首頁',
    },
    
    // 页脚
    footer: {
      companyName: '株式會社KANAE',
      slogan: '追求物質與精神的雙重幸福，以利他之心創造通往世界的價值',
      license: '東京都知事(1)第107157號',
      businessContent: '業務內容',
      rentalBusiness: '租賃業務',
      salesBusiness: '買賣業務',
      managementBusiness: '管理業務',
      minpakuBusiness: '民宿業務',
      companyInfo: '公司信息',
      aboutUs: '公司簡介',
      philosophy: '企業理念',
      contact: '聯繫我們',
      privacyPolicy: '隱私政策',
      contactInfo: '聯繫我們',
      terms: '使用條款',
      sitemap: '網站地圖',
      allRightsReserved: 'All rights reserved.',
    },
    
    // 投资物业搜索
    properties: {
      title: '投資物業資訊',
      subtitle: '租賃、買賣、投資物業統合搜索，以投資物業為主推薦',
      label: 'Properties',
      all: '全部',
      rent: '租賃',
      sale: '買賣',
      invest: '投資物業',
      searchTitle: '搜索物業',
      searchPlaceholder: '按地區、線路、車站搜索',
      searchButton: '搜索',
      filterByType: '按物業類型篩選',
      quickSearch: '快速搜索',
      detailedSearch: '詳細搜索',
      showAllProperties: '顯示全部物業',
      propertyView: '物業列表',
      wardView: '按區域搜索',
      totalResults: '符合條件的物業：',
      resultsCount: '件',
      activeFilters: '個條件',
      clearAll: '清除全部',
      geography: '地理位置',
      ward: '區',
      selectWard: '全部區域',
      railway: '路線',
      selectRailway: '全部路線',
      station: '車站',
      selectStation: '全部車站',
      budget: '價格範圍',
      minPrice: '下限（萬日元）',
      maxPrice: '上限（萬日元）',
      pricePlaceholderMin: '例：3000',
      pricePlaceholderMax: '例：10000',
      propertyType: '物業類型',
      wholeBuilding: '整棟大廈',
      wholeOffice: '整棟辦公樓',
      wholeApartment: '整棟公寓',
      incomeProperty: '整棟收益物業',
      area: '面積',
      landArea: '土地面積（㎡）',
      buildingArea: '建築面積（㎡）',
      areaMin: '下限',
      areaMax: '上限',
      advancedFilters: '詳細條件',
      expectedYield: '預期收益率（%）',
      yieldMin: '下限（例：4.0）',
      yieldMax: '上限（例：10.0）',
      buildingAge: '樓齡',
      ageNotSpecified: '不限',
      ageWithin5: '5年以內',
      ageWithin10: '10年以內',
      ageWithin20: '20年以內',
      ageWithin30: '30年以內',
      walkTime: '步行時間',
      walkNotSpecified: '不限',
      walk5min: '5分鐘以內',
      walk10min: '10分鐘以內',
      walk15min: '15分鐘以內',
      walk20min: '20分鐘以內',
      reset: '重置',
      sortBy: '排序：',
      byPropertyType: '物業類型：',
      monthlyRent: '月租金額',
      salePrice: '銷售價格',
      grossYield: '表面收益率',
      recommended: '推薦物業',
      viewDetails: '查看詳情',
      favorite: '收藏',
      loadMore: '查看更多',
      noResults: '未找到符合條件的物業',
      rentBadge: '租賃',
      saleBadge: '買賣',
      investBadge: '投資',
      areaLabel: '所在地',
      nearestStation: '最近車站',
      minutesWalk: '步行分鐘',
      layout: '戶型',
      built: '竣工年月',
      units: '總戶數',
      unitsSuffix: '戶',
    },

    // 通用
    common: {
      readMore: '瞭解詳情',
      backToTop: '返回頂部',
      loading: '加載中...',
      error: '發生錯誤',
      success: '成功',
      confirm: '確認',
      cancel: '取消',
      save: '保存',
      delete: '刪除',
      edit: '編輯',
      search: '搜索',
      close: '關閉',
    },
  },
  
  en: {
    // Navigation
    nav: {
      home: 'Home',
      propertyInfo: 'Property Info',
      rent: 'Rental',
      sale: 'Sales',
      invest: 'Investment Properties',
      properties: 'Investment Properties',
      management: 'Management',
      minpaku: 'Vacation Rental',
      market: 'Market Insight',
      about: 'About Us',
      philosophy: 'Philosophy',
      contact: 'Contact',
    },
    
    // Hero Section
    hero: {
      companyType: 'Co., Ltd.',
      title: 'Creating World-Class Value',
      subtitle: 'With Pursuit of Material and Spiritual Happiness',
      cta: 'Search Properties',
      learnMore: 'Learn More',
      searchRent: 'Search Rental Properties',
      searchSale: 'Search Properties for Sale',
      searchInvest: 'Search Investment Properties',
      searchMinpaku: 'Vacation Rental Consultation',
      searchPlaceholder: 'Search by area, line, or station',
      searchButton: 'Search',
    },
    
    // Story (Architecture & Nature)
    story: {
      label: 'Our Perspective',
      title: 'Where Architecture and Nature Quietly Meet',
      lead: 'A home is more than walls and a roof. Morning light, the path of the wind, greenery beyond the window, the patina of time on materials — all of these shape the way we live.',
      body: 'We see property not as a container, but as a place that grows with time. The shifting of light and shadow, the presence of water and plants, the texture of materials under your hand. With attention to these details, we propose homes that stay close to how each person truly lives.',
      item1Title: 'Light & Shadow',
      item1Text: 'The passage of daylight quietly changes the expression of a room.',
      item2Title: 'Material & Time',
      item2Text: 'Wood and stone grow deeper and warmer with the years.',
      item3Title: 'Green & Water',
      item3Text: 'Plants by the window, the presence of water — nature lets a home breathe.',
      caption1: 'Dappled light — traces of time',
      caption2: 'Texture — expressions carved by years',
      caption3: 'Mossed stone — time quietly accumulating',
    },

    // Architecture & People — the philosophy of space
    dialogue: {
      label: 'Architecture & People',
      title: 'Architecture and people,\nin quiet dialogue',
      lead: 'A space reflects the presence of those who live within it. A single chair someone left behind, a sprig of green casually placed. Between architecture, people, and nature, we listen for the wordless dialogue.',
      items: [
        {
          eyebrow: 'Human Traces — Negative Space',
          title: 'Someone was here',
          text: 'A single chair, a draped cloth. Even after one departs, warmth and presence linger in the room. Negative space is where that memory breathes.',
        },
        {
          eyebrow: 'Mind & Space in Dialogue',
          title: 'A great nature within a small garden',
          text: 'Ripples raked into sand, a stone standing still. Through the act of tending, the vastness of nature rises instead. A stillness within order that draws one inward.',
        },
        {
          eyebrow: 'Honesty of Material — Time',
          title: 'Stone speaks not, yet marks time',
          text: 'The grain of unpolished stone, shadows drawn by light. In its raw form, material quietly tells of geological time and the weight of years.',
        },
        {
          eyebrow: 'Design in Harmony with Nature',
          title: 'The more you pare away, the more space lives',
          text: 'A refined vessel, a fractured stone, a budding branch. The intent of the human hand resonates with nature\u2019s imperfection. Minimal intervention lets form come alive.',
        },
      ],
    },
    
    // Services
    services: {
      title: 'Our Services',
      subtitle: 'Four Business Areas to Realize Customer Happiness',
      learnMore: 'Learn More',
      dialogueLead: 'The forms of our work may differ, yet what we look to is always the same \u2014 the quiet dialogue exchanged between architecture and people.',
      rent: {
        title: 'Rental Business',
        philosophy: 'Connecting people to where beginnings take root',
        description: 'Property search service tailored to customer needs by area, transit line, rent, layout, etc.',
      },
      sale: {
        title: 'Sales Business',
        philosophy: 'Passing the vessel of living on to the next life',
        description: 'Comprehensive support for real estate transactions from condos, houses, land to investment properties',
      },
      management: {
        title: 'Management Business',
        philosophy: 'Guarding and nurturing a home\u2019s value through the years',
        description: 'Comprehensive management services for rental properties to protect owner asset value',
      },
      minpaku: {
        title: 'Vacation Rental Business',
        philosophy: 'Binding each visitor to the time of this land',
        description: 'Full support for vacation rental operations with OneStep PMS integration to maximize revenue',
      },
      stats: {
        minpakuProperties: 'Vacation Rental Properties',
        managedProperties: 'Managed Properties',
        customerSmiles: 'Customer Smiles',
      },
    },
    
    // Philosophy
    philosophy: {
      title: 'Our Philosophy',
      subtitle: 'Our Guiding Principles and Vision',
      heroTitle: 'Our Philosophy',
      heroSubtitle: 'Creating world-class value with pursuit of material and spiritual happiness',
      missionCardTitle: 'Pursuing Material and Spiritual Happiness',
      missionCardDesc: 'We realize material and spiritual prosperity for our customers, employees, and society as a whole, contributing to the happiness of all stakeholders.',
      missionCheck1: 'Customer satisfaction and happiness first',
      missionCheck2: 'Employee growth and job fulfillment',
      missionCheck3: 'Contribution and value creation for society',
      visionCardTitle: 'Creating Value with Altruism',
      visionCardDesc: 'With an altruistic spirit, we create world-class value and aim to become a leading company in the real estate industry.',
      visionCheck1: 'Cherish consideration for others',
      visionCheck2: 'Deliver innovative services',
      visionCheck3: 'Set the industry standard',
      guidelinesTitle: 'Action Guidelines',
      guideline1Title: 'Integrity',
      guideline1Desc: 'We put integrity first in all actions. We strive to be honest and transparent with customers, business partners, and society.',
      guideline2Title: 'Challenge',
      guideline2Desc: 'We never settle for the status quo and always challenge ourselves to create new value. We embrace learning and are not afraid to fail.',
      guideline3Title: 'Gratitude',
      guideline3Desc: 'We are grateful for every encounter and connection. We never forget to appreciate our customers, colleagues, and society.',
      promisesTitle: 'Our Promises',
      promiseCustomerTitle: 'Promise to Customers',
      promiseCustomerItems: [
        'Provide the best service and satisfaction',
        'Offer proposals from the customer\'s perspective',
        'Respond quickly and attentively',
        'Value long-term relationship building',
      ],
      promiseSocietyTitle: 'Promise to Society',
      promiseSocietyItems: [
        'Practice legal compliance and ethical management',
        'Promote environmentally conscious business activities',
        'Contribute to the development of local communities',
        'Work toward a sustainable society',
      ],
      promiseEmployeeTitle: 'Promise to Employees',
      promiseEmployeeItems: [
        'Provide a fulfilling work environment',
        'Offer fair evaluation and growth opportunities',
        'Respect work-life balance',
        'Embrace diversity and leverage individual strengths',
      ],
      ourPhilosophyTitle: 'Our Philosophy',
      motto: 'Integrity & Unity of Knowledge and Action',
      mottoDescription: 'Maintain an honest heart and always practice what we learn',
      vision: 'Vision',
      visionText: 'To become a globally competitive brand and achieve sustainable development',
      mission: 'Mission',
      missionText: 'Creating customer smiles with speed, affordability, and convenience that surpass others',
      values: 'Values',
      principlesTitle: '7 Management Principles',
      principles: [
        'Pursue Happiness for All Employees',
        'Do What is Right as a Human Being',
        'Build Win-Win Relationships',
        'Maximize Sales, Minimize Expenses',
        'Participatory Management for All',
        'Make Unrelenting Efforts',
        'Stay Positive and Optimistic',
      ],
      missionPoints: [
        'Understand needs through deep insight',
        'Deliver with speed, affordability, and convenience',
        'Create inspiring experiences and loyal fans',
      ],
      visionPoints: [
        'Become a global brand',
        'Build a foundation for 100 years',
        'Always evolving organization',
      ],
      valuesPoints: [
        'Pursue material and spiritual happiness',
        'Contribute to society with altruism',
        'Make unrelenting efforts',
      ],
      practiceTitle: 'Philosophy in Practice',
      practiceDescription: 'We practice this philosophy in our daily work, pursuing happiness for our customers, society, and all employees.',
      practiceStats: {
        satisfaction: 'Customer Satisfaction Goal',
        support: 'Support System',
        languages: 'Language Support',
        growth: 'Growth Potential',
      },
      ctaTitle: 'Join Our Team',
      ctaDescription: 'We are looking for partners who resonate with KANAE\'s philosophy and want to create value together.',
      ctaContact: 'Contact Us',
      ctaAbout: 'View Company Profile',
    },
    
    // About Us
    about: {
      title: 'About Us',
      subtitle: 'Creating world-class value with pursuit of material and spiritual happiness',
      heroTitle: 'About Us',
      heroSubtitle: 'Creating world-class value with pursuit of material and spiritual happiness',
      companyInfoTitle: 'Company Information',
      strengthsTitle: 'Our Strengths',
      businessContentTitle: 'Business Content',
      service1Title: 'Rental Business',
      service1Desc: 'We propose the best rental properties tailored to each customer\'s lifestyle.',
      service2Title: 'Sales Business',
      service2Desc: 'From real estate sales brokerage to investment property introductions, we cover it all.',
      service3Title: 'Management Business',
      service3Desc: 'We properly manage owners\' valuable assets and support stable revenue.',
      service4Title: 'Vacation Rental Business',
      service4Desc: 'We leverage our vacation rental expertise to support maximum revenue.',
      ourPhilosophyTitle: 'Our Philosophy',
      ourPhilosophySubtitle: 'Creating world-class value with pursuit of material and spiritual happiness',
      ourPhilosophyButton: 'View Our Philosophy',
      accessTitle: 'Access',
      locationTitle: 'Location',
      accessDetailTitle: 'Access Information',
      contactSectionTitle: 'Contact Us',
      contactSectionDesc: 'For questions about properties or services, please feel free to contact us.',
      contactButton: 'Contact Us Here',
      companyInfo: 'Company Information',
      companyName: 'Company Name',
      representative: 'Representative',
      established: 'Established',
      corporateNumber: 'Corporate Number',
      address: 'Head Office',
      phone: 'Phone',
      email: 'Email',
      website: 'Website',
      businessHours: 'Business Hours',
      businessHoursValue: 'Weekdays 9:00-18:00 / Saturday 10:00-17:00 (Closed Sundays & Holidays)',
      businessContent: 'Business Content',
      businessContentValue: 'Real Estate Rental Brokerage, Sales Brokerage, Rental Management, Vacation Rental Operations, Real Estate Consulting',
      licenseNumber: 'License Number',
      licenseNumberValue: 'Real Estate Transaction Business Tokyo Governor (1) No. 107157',
      associations: 'Associations',
      associationsValue: 'Japan Real Estate Transaction Association, Tokyo Real Estate Transaction Association',
      strengths: 'Our Strengths',
      strength1Title: 'Community-Based Attentive Service',
      strength1Description: 'Our staff, knowledgeable about the Tokyo 23 wards area, propose optimal properties tailored to each customer\'s needs.',
      strength2Title: 'Multilingual Support (Japanese, Chinese, English)',
      strength2Description: 'We have staff proficient in Chinese and English to serve international clients with confidence.',
      strength3Title: 'Leveraging Latest Technology',
      strength3Description: 'We support efficient property searches with AI-powered matching systems and online viewing services.',
      strength4Title: 'One-Stop Service',
      strength4Description: 'Our comprehensive strength lies in handling all real estate needs - rental, sales, management, and vacation rentals - in one place.',
      access: 'Access',
      location: 'Location',
      accessInfo: 'Access Information',
      nearStation1: 'JR/Tokyo Metro Takadanobaba Station',
      nearStation1Time: '10 min walk from Waseda Exit',
      nearStation2: 'Tokyo Metro Fukutoshin Line Nishi-Waseda Station',
      nearStation2Time: '8 min walk from Exit 3',
    },
    
    // Sales Search
    sale: {
      title: 'Property Sales Search',
      subtitle: 'Proposing high asset-value properties including apartments, houses, and land',
      searchTitle: 'Search Properties',
      propertyType: 'Property Type',
      apartment: 'Apartment',
      house: 'House',
      land: 'Land',
      location: 'Location',
      locationPlaceholder: 'Select a location',
      minPrice: 'Price (Min)',
      maxPrice: 'Price (Max)',
      pricePlaceholder: 'e.g., 30000000',
      searchButton: 'Search with these conditions',
    },
    
    // Investment Property Search
    properties: {
      title: 'Investment Property Information',
      subtitle: 'Unified search for rental, sales, and investment properties, with a focus on investment properties',
      label: 'Properties',
      all: 'All',
      rent: 'Rental',
      sale: 'Sale',
      invest: 'Investment',
      searchTitle: 'Find a Property',
      searchPlaceholder: 'Search by area, line, or station',
      searchButton: 'Search',
      filterByType: 'Filter by property type',
      quickSearch: 'Quick Search',
      detailedSearch: 'Detailed Search',
      showAllProperties: 'Show All Properties',
      propertyView: 'Property List',
      wardView: 'Search by Ward',
      totalResults: 'Properties found: ',
      resultsCount: ' results',
      activeFilters: ' filters active',
      clearAll: 'Clear All',
      geography: 'Location',
      ward: 'Ward',
      selectWard: 'All Wards',
      railway: 'Railway Line',
      selectRailway: 'All Lines',
      station: 'Station',
      selectStation: 'All Stations',
      budget: 'Price Range',
      minPrice: 'Min (10k JPY)',
      maxPrice: 'Max (10k JPY)',
      pricePlaceholderMin: 'e.g. 3000',
      pricePlaceholderMax: 'e.g. 10000',
      propertyType: 'Property Type',
      wholeBuilding: 'Whole Apartment Building',
      wholeOffice: 'Whole Office Building',
      wholeApartment: 'Whole Apartment Complex',
      incomeProperty: 'Whole Income Property',
      area: 'Area',
      landArea: 'Land Area (㎡)',
      buildingArea: 'Building Area (㎡)',
      areaMin: 'Min',
      areaMax: 'Max',
      advancedFilters: 'Advanced Filters',
      expectedYield: 'Expected Yield (%)',
      yieldMin: 'Min (e.g. 4.0)',
      yieldMax: 'Max (e.g. 10.0)',
      buildingAge: 'Building Age',
      ageNotSpecified: 'Not Specified',
      ageWithin5: 'Within 5 years',
      ageWithin10: 'Within 10 years',
      ageWithin20: 'Within 20 years',
      ageWithin30: 'Within 30 years',
      walkTime: 'Walk Time',
      walkNotSpecified: 'Not Specified',
      walk5min: 'Within 5 min',
      walk10min: 'Within 10 min',
      walk15min: 'Within 15 min',
      walk20min: 'Within 20 min',
      reset: 'Reset',
      sortBy: 'Sort by: ',
      byPropertyType: 'Property Type: ',
      monthlyRent: 'Monthly Rent',
      salePrice: 'Sale Price',
      grossYield: 'Gross Yield',
      recommended: 'Recommended Properties',
      viewDetails: 'View Details',
      favorite: 'Favorite',
      loadMore: 'Load More',
      noResults: 'No properties match your criteria',
      rentBadge: 'Rental',
      saleBadge: 'Sale',
      investBadge: 'Investment',
      areaLabel: 'Location',
      nearestStation: 'Nearest Station',
      minutesWalk: 'min walk',
      layout: 'Layout',
      built: 'Built',
      units: 'Total Units',
      unitsSuffix: '',
    },

    // Vacation Rental
    minpaku: {
      title: 'Vacation Rental Management Service',
      subtitle: 'Operate your property as a vacation rental and achieve stable revenue',
      calculator: 'Revenue Simulator',
      calculatorDesc: 'Easily simulate the revenue from operating your property as a vacation rental',
      propertyType: 'Property Type',
      area: 'Area (㎡)',
      areaPlaceholder: '50',
      rooms: 'Layout',
      nightlyRate: 'Nightly Rate',
      nightlyRatePlaceholder: '10000',
      occupancyRate: 'Occupancy Rate (%)',
      occupancyRatePlaceholder: '70',
      managementFee: 'Management Fee Rate (%)',
      managementFeePlaceholder: '25',
      calculate: 'Calculate Revenue',
      results: 'Revenue Simulation Results',
      income: 'Income',
      grossRevenue: 'Gross Revenue',
      bookedNights: 'Booked Nights',
      nights: 'nights',
      expenses: 'Expenses',
      managementFeeLabel: 'Management Fee',
      cleaningFee: 'Cleaning Fee',
      utilities: 'Utilities',
      platformFee: 'Platform Fee',
      totalExpenses: 'Total Expenses',
      netRevenue: 'Net Revenue',
      perMonth: '/ month',
      servicesTitle: 'Vacation Rental Management Services',
      service1: 'OneStep PMS Integration',
      service1Desc: 'Auto-sync with Airbnb, Booking.com, etc.',
      service2: '24/7 Guest Support',
      service2Desc: 'Multilingual guest support',
      service3: 'Cleaning & Maintenance',
      service3Desc: 'Professional cleaning team',
      service4: 'Revenue Optimization',
      service4Desc: 'AI-powered pricing for maximum revenue',
    },
    
    // Rental Search
    rent: {
      title: 'Rental Property Search',
      subtitle: 'Find your ideal rental property that fits your lifestyle',
      searchTitle: 'Search Properties',
      area: 'Area',
      areaPlaceholder: 'Select an area',
      minRent: 'Rent (Min)',
      maxRent: 'Rent (Max)',
      layout: 'Layout',
      features: 'Special Features',
      searchButton: 'Search with these conditions',
      recommended: 'Recommended Properties',
      perMonth: '/ month',
      viewDetails: 'View Details',
      favorite: 'Favorite',
      loadMore: 'Load More',
      rentalProcess: 'Rental Contract Process',
      step1: 'Property Search',
      step1Desc: 'Search properties with desired conditions',
      step2: 'Viewing Reservation',
      step2Desc: 'Schedule viewing for interesting properties',
      step3: 'Application',
      step3Desc: 'Submit application for screening',
      step4: 'Contract Procedures',
      step4Desc: 'Important matters explanation and contract signing',
      step5: 'Move-in',
      step5Desc: 'Key handover and move-in',
    },
    
    // Management
    management: {
      title: 'Rental Management Service',
      subtitle: 'Providing optimal rental management for both owners and tenants',
      forOwners: 'For Owners',
      forTenants: 'For Tenants',
      ownerServices: 'Management Services for Owners',
      feature1: 'Stable Revenue Management',
      feature1Desc: 'Secure revenue from rent collection to delinquency handling.',
      feature2: 'Building & Equipment Management',
      feature2Desc: 'Maintain property value from regular inspections to trouble response.',
      feature3: 'Tenant Management',
      feature3Desc: 'Comprehensive support from tenant recruitment to contract renewal.',
      rentManagement: 'Rent Management Service',
      buildingManagement: 'Building Management Service',
      tenantManagement: 'Tenant Management Service',
      otherServices: 'Other Services',
      managementFees: 'Management Fees',
      basicPlan: 'Basic Management Plan',
      fullPlan: 'Full Management Plan',
      premiumPlan: 'Premium Plan',
      recommended: 'Recommended',
      managementStats: 'Management Track Record',
      properties: 'Properties Managed',
      units: 'Units Managed',
      occupancyRate: 'Occupancy Rate',
      avgYears: 'Average Management Years',
      tenantServices: 'Tenant Services',
      comfortableSupport: 'Comfortable Living Environment Support',
      supportDesc: 'We fully support your comfortable living from troubleshooting during occupancy to move-out procedures.',
      contactWindow: 'Contact Window',
      businessHours: 'Business Hours',
      emergencyContact: 'Emergency Contact (24/7)',
      faq: 'FAQ',
      ctaTitle: 'Feel Free to Consult About Management',
      ctaDescription: 'Consultations about property management are free. Please feel free to contact us.',
      ctaButton: 'Contact Us',
    },
    
    // Call to Action
    cta: {
      title: 'Make Your Dreams Come True,',
      subtitle: 'Your Best Partner Is Here',
      description: 'From rental, sales, management to vacation rental, we provide one-stop service for all real estate needs. Customer happiness is our priority, delivering honest and reliable service.',
      searchProperties: 'Search Properties',
    },
    
    // Features
    features: {
      title: 'Why Choose KANAE',
      subtitle: 'Putting customer happiness first and providing high-quality services',
      items: [
        {
          title: 'Multilingual Support',
          description: 'Support in Japanese, Chinese, and English for international customers',
          iconPath: 'M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9',
        },
        {
          title: '24/7 Response',
          description: 'Online inquiries accepted 24/7. Complete emergency support system',
          iconPath: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z',
        },
        {
          title: 'Reliable Support',
          description: 'Qualified real estate agents and architects provide expert support',
          iconPath: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z',
        },
        {
          title: 'IT Integration',
          description: 'Seamless contract and management through integration with leading platforms',
          iconPath: 'M13 10V3L4 14h7v7l9-11h-7z',
        },
        {
          title: 'Transparent Pricing',
          description: 'Instant verification of initial costs and tax calculations. No hidden fees',
          iconPath: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
        },
        {
          title: 'One-Stop Service',
          description: 'All real estate needs from rental, sales, management to vacation rental under one roof',
          iconPath: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z',
        },
      ],
    },
    
    // Statistics
    stats: {
      title: 'KANAE by Numbers',
      subtitle: 'Steady growth and proven track record',
      items: [
        {
          number: '30+',
          label: 'Vacation Rental Properties',
          description: 'High profitability through OneStep PMS',
        },
        {
          number: '500+',
          label: 'Rental & Sales Properties',
          description: 'Reliable support with abundant experience',
        },
        {
          number: '1000+',
          label: 'Happy Customers',
          description: '95%+ satisfaction rating',
        },
        {
          number: '3 Years',
          label: 'Since Establishment',
          description: 'Founded in 2021, steady growth',
        },
      ],
      banksTitle: 'Major Partner Banks',
      banksSubtitle: 'Strong partnerships for secure transactions',
      banks: [
        'SBJ Bank',
        'Star Bank',
        'Sony Bank',
        'Asuka Credit Union',
        'SMBC Trust Bank',
        'Sumitomo Mitsui Banking',
      ],
    },
    
    // Contact
    contact: {
      title: 'Contact Us',
      subtitle: 'Feel free to reach out',
      phone: 'Phone Inquiry',
      email: 'Email Inquiry',
      businessHours: 'Business Hours',
      businessHoursValue: 'Weekdays 9:00-18:00',
      businessHoursNote: '(Weekends & holidays by appointment)',
      address: 'Address',
      inquiryButton: 'Contact Us',
      onlineNote: '※ Online consultation and viewing reservations accepted 24/7',
      formTitle: 'Contact Form',
      formDescription: 'Please fill out the form below and submit. Our staff will contact you within 3 business days.',
      name: 'Name',
      namePlaceholder: 'John Doe',
      nameRequired: 'Please enter your name',
      emailLabel: 'Email Address',
      emailPlaceholder: 'example@email.com',
      emailRequired: 'Please enter your email address',
      phoneLabel: 'Phone Number',
      phonePlaceholder: '03-1234-5678',
      phoneOptional: 'Optional',
      inquiryType: 'Inquiry Type',
      inquiryTypeViewing: 'Viewing Appointment',
      inquiryTypeInquiry: 'General Inquiry',
      inquiryTypeApplication: 'Rental Application',
      inquiryTypeOther: 'Other',
      message: 'Message',
      messagePlaceholder: 'Please enter your inquiry',
      messageRequired: 'Please enter your message',
      submit: 'Submit',
      submitting: 'Submitting...',
      successTitle: 'Submitted Successfully',
      successMessage: 'Thank you for your inquiry. Our staff will contact you within 3 business days.',
      errorTitle: 'Submission Error',
      errorMessage: 'Failed to submit. Please contact us by phone (03-6914-3633) or email (info@kanae-tokyo.com).',
      backToHome: 'Back to Home',
    },
    
    // Footer
    footer: {
      companyName: 'KANAE CO., LTD.',
      slogan: 'Creating world-class value with pursuit of material and spiritual happiness',
      license: 'Tokyo Governor License (1) No. 107157',
      businessContent: 'Business Content',
      rentalBusiness: 'Rental Business',
      salesBusiness: 'Sales Business',
      managementBusiness: 'Management Business',
      minpakuBusiness: 'Vacation Rental',
      companyInfo: 'Company Info',
      aboutUs: 'About Us',
      philosophy: 'Philosophy',
      contact: 'Contact',
      privacyPolicy: 'Privacy Policy',
      contactInfo: 'Contact',
      terms: 'Terms of Service',
      sitemap: 'Sitemap',
      allRightsReserved: 'All rights reserved.',
    },
    
    // Common
    common: {
      readMore: 'Read More',
      backToTop: 'Back to Top',
      loading: 'Loading...',
      error: 'An error occurred',
      success: 'Success',
      confirm: 'Confirm',
      cancel: 'Cancel',
      save: 'Save',
      delete: 'Delete',
      edit: 'Edit',
      search: 'Search',
      close: 'Close',
    },
  },
};

export type Locale = 'ja' | 'zh' | 'en';
export type Translations = typeof translations.ja;
